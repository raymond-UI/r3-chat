 ...
 * Use the unified `useChat` hook from `@/hooks/useChat` instead.
 * 
 * The useChat hook now provides all functionality including:
 * - AI streaming and responses
 ...
 * 
 * Migration guide:
 * - Replace `useAI()` with `useChat({ conversationId })`
 * - Replace `streamToAI()` with `submitWithFiles()` or `handleSubmit()`
 * - Replace `generateTitle()` with the same function from useChat
 * - Replace `fetchModels()` with the same function from useChat
 */
...
//           model: model || selectedModel,
//           userId: user.id,
//           fileIds: fileIds || [], // Include file IDs in request
//         }),
//       });
