...
import type { ModelAvailability } from "@/types/providers";
...
export function ModelSelectorEnhanced({ 
  ...
  const getSourceIcon = (source: ModelAvailability['currentSource']) => {
    switch (source) {
      case 'user-direct':
        return <Shield className="h-3 w-3 text-green-600" />;
      case 'user-openrouter':
  ...
  const getSourceBadge = (source: ModelAvailability['currentSource']) => {
    switch (source) {
      case 'user-direct':
        return <Badge variant="outline" className="text-xs text-green-600">Direct</Badge>;
      case 'user-openrouter':
        return <Badge variant="outline" className="text-xs text-blue-600">Your OR</Badge>;
      case 'system-default':
              ...
              <SelectItem key={model.modelId} value={model.modelId}>
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
                    {getSourceIcon(model.currentSource)}
