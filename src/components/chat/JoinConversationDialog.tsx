...
export function JoinConversationDialog({
  ...
  const { user } = useUser();
  const joinViaInvite = useMutation(api.conversations.joinViaInvite);
  const [isJoining, setIsJoining] = useState(false);
  const [joinError, setJoinError] = useState<string | null>(null);

  // Get invite details instead of conversation details directly
  const inviteResult = useQuery(
    api.conversations.getInviteDetails,
    inviteCode ? { inviteCode } : "skip"
  );

  const conversation = inviteResult?.success ? inviteResult.conversation : null;
  ...
  const isAlreadyParticipant = conversation?.participants.includes(
    user?.id || ""
  );
  const activeUsers =
    presence
  ...
  const handleJoin = async () => {
    ...
    try {
      // Use the new joinViaInvite mutation
      const result = await joinViaInvite({ inviteCode });
      
      // Join the conversation
      onJoin(result.conversationId);
      onClose();
    } catch (error) {
      console.error("Failed to join conversation:", error);
      setJoinError(error instanceof Error ? error.message : "Failed to join conversation. Please try again.");
    } finally {
      setIsJoining(false);
    }
  };
  ...
  if (inviteCode && inviteResult === undefined) {
    return (
  ...
  if (inviteCode && inviteResult && !inviteResult.success) {
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-md">
          <DialogHeader>
                ...
                {conversation?.participants
                  .filter(
                    (participantId, index, self) =>
                      self.indexOf(participantId) === index
                  ) // Remove duplicates
                  .slice(0, 5)
